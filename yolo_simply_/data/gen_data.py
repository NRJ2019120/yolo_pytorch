...
class gen_Data:
    def __init__(self,xml_path,img_path,save_img_224_path,save_txt):
        ...
        # self.xml_list = self.xml_list[:15]   #测试１５个
        # print(len(self.xml_list))
        # exit()
        self.read_xml()
            ...
            try:
                ...
                try:
                    ...
                    for obj in root.iter("object"):
                        ...
                        x1 = int(int(obj.find("bndbox/xmin").text) * 224 / img_width)  #变换成224缩放后的坐标
                        ...
                        cx = int((x1 + x2)/2)          #需要int ，可以不转成int
                        cy = int((y1 + y2)/2)

                        cbox_width_index = cx // 32    #224*7==224  判断目标中心属于第几个cell（32*32）
                        cbox_height_index = cy // 32

                        list_width.append(cbox_width_index)
                        list_heigth.append(cbox_height_index)

                        box_cx = cbox_width_index * 32 + 16      #cell中心坐标
                        box_cy = cbox_height_index * 32 + 16

                        boxes = []
                         #设置anchor 框
                        _c_16_x1 = box_cx - 16             # 通道 0　　cell 32*32
                        _c_16_y1 = box_cy - 16
                        _c_16_x2 = box_cx + 16
                        _c_16_y2 = box_cy + 16
                        ...
                        _c_32_x1 = box_cx - 32             # 通道 2   cell64*64
                        _c_32_y1 = box_cy - 32
                        _c_32_x2 = box_cx + 32
                        _c_32_y2 = box_cy + 32
                        ...
                        index_max = np.where(iou == np.max(iou))[0][0]   #挑出iou最大的anchor
    ...
    def get_offset(self,box,max_box):
        # 计算公式，(实际框-定义框)/实际框的宽和高　　　　　　　　　　　 不一定能压缩到（－１，１）　之间
...
if __name__ == '__main__':
    ...
    # save_txt = r"/media/tensorflow01/myfile/0_yolo_VOC/2012_train/label.txt"   #直接添加

    xml_path = r"/media/tensorflow01/myfile/0_yolo_VOC/2007_train/Annotations"  #增加训练样本
    img_path = r"/media/tensorflow01/myfile/0_yolo_VOC/2007_train/JPEGImages"
    save_img_224_path = r"/media/tensorflow01/myfile/0_yolo_VOC/2007_train/224"
    save_txt = r"/media/tensorflow01/myfile/0_yolo_VOC/2007_train/2012+2007_train_label.txt"  #2012 train + 2007train lebal
    # save_txt = r"/media/tensorflow01/myfile/0_yolo_VOC/2012_test/2012_all_label.txt"  #2012 train + 2012test lebal
